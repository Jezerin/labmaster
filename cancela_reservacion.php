...
$query	=mysql_query("SELECT*FROM reservaciones WHERE ID_USUARIO=$id");
	...
	{	
		...
		{	
			if($row[5]==1)
			{	
				echo"<form action=cancela_r.php method=post>";
				echo"<td>$row[1]</td><td>$row[3]</td><td>$row[2]</td>";
				echo"<input type=text style=visibility:hidden name=id_reservacion value=$row[0]>";
				echo"<td><input type=submit value=Cancelar></td></form>";
				echo"</tr>";
			}
		}
