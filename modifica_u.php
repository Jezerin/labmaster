...
$flag_password	=0;
	...
	{ 
		echo"Alguno de los datos no ha sido introducido, por favor llene todos los campos para el registro<a href=modifica_usuario.php>Reintentar</a>";
    }
	...
	if($flag_nombre==1 && $flag_apellido==1 && $flag_mail==1 && $flag_password==1)
	{
		...
		mysql_query("UPDATE usuarios SET NOMBRE='$nombre', APELLIDO='$apellido', EMAIL='$email', USUARIO='$usuario', PASSWORD='$password', ROL='$rol' WHERE ID_USUARIO=$id",$link);
		
		//echo"$id   $nombre   $apellido   $email   $usuario   $password";
